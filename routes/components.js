...
router.get("/all/:category", function (req, res) {
    if (req.params.category) {
        var Components_1 = [];
        var ComponentsLast = [];
        var Path = "".concat(__dirname.replace("routes", ""), "/public/components/").concat(req.params.category);
...
router.get("/iframe/:category/:name", function (req, res) {
    if (req.params.category && req.params.name) {
        var Path = "".concat(__dirname.replace("routes", ""), "/public/components/").concat(req.params.category, "/").concat(req.params.name, ".html");
        if (fs.existsSync(Path)) {
...
router.get("/thumbnail/:category/:name", function (req, res) {
    ...
    var Path = "".concat(__dirname.replace("routes", ""), "/public/components/").concat(req.params.category, "/").concat(req.params.name, ".jpg");
    if (req.params.category) {
        if (fs.existsSync(Path)) {
            res.sendFile(Path);
        }
